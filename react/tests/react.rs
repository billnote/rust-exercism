...
fn compute_cells_take_inputs_in_the_right_order() {
    ...
    let output = reactor
        .create_compute(&vec![one, two], |v| v[0] + v[1] * 10)
        .unwrap();
...
fn do_not_break_cell_if_creating_compute_cell_with_valid_and_invalid_input() {
    ...
    assert!(
        reactor
            .create_compute(&vec![input, dummy_cell], |_| 0)
            .is_err()
    );
...
fn compute_cells_can_depend_on_other_compute_cells() {
    ...
    let output = reactor
        .create_compute(&vec![times_two, times_thirty], |v| v[0] + v[1])
        .unwrap();
...
fn error_adding_callback_to_nonexistent_cell() {
    ...
    assert!(
        Reactor::new()
            .add_callback(output, |_: usize| println!("hi"))
            .is_err()
    );
}
...
fn callbacks_only_fire_on_change() {
    let mut values = Vec::new();
    {
        ...
        let input = reactor.create_input(1);
        let output = reactor
            .create_compute(&vec![input], |v| if v[0] < 3 { 111 } else { 222 })
            .unwrap();
        assert!(reactor.add_callback(output, |v| values.push(v)).is_ok());
...
fn callbacks_should_only_be_called_once_even_if_multiple_dependencies_change() {
    let mut values = Vec::new();
    {
        ...
        let minus_one2 = reactor
            .create_compute(&vec![minus_one1], |v| v[0] - 1)
            .unwrap();
        let output = reactor
            .create_compute(&vec![plus_one, minus_one2], |v| v[0] * v[1])
            .unwrap();
        assert!(reactor.add_callback(output, |v| values.push(v)).is_ok());
        assert!(reactor.set_value(input, 4).is_ok());
    }
...
fn callbacks_should_not_be_called_if_dependencies_change_but_output_value_doesnt_change() {
    let mut values = Vec::new();
    {
        ...
        let always_two = reactor
            .create_compute(&vec![plus_one, minus_one], |v| v[0] - v[1])
            .unwrap();
...
fn test_adder_with_boolean_values() {
    ...
    let a_xor_b = reactor
        .create_compute(&vec![a, b], |v| v[0] ^ v[1])
        .unwrap();
    let sum = reactor
        .create_compute(&vec![a_xor_b, carry_in], |v| v[0] ^ v[1])
        .unwrap();
    ...
    let carry_out = reactor
        .create_compute(&vec![a_xor_b_and_cin, a_and_b], |v| v[0] || v[1])
        .unwrap();
